...
popUpCal.prototype = {
    ...
    setPos: function(inputObj){
        ...
        var tWidth = $(inputObj).getWidth();
        $(this.calendarId).setStyle({left: (tPos.left+tWidth/2 +10) + 'px', top: (tPos.top+tHeight+4) + 'px'});
        $(this.calendarId).show();
        if(Prototype.Browser.IE){
           $(this.calendarId).setStyle({zIndex: 99});
           $(this.calendarId + 'Frame').absolutize();
           $(this.calendarId + 'Frame').setStyle({left: (tPos.left+tWidth/2 +10) + 'px', top: (tPos.top + tHeight+4) + 'px'});
