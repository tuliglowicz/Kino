...
privileges = Privilege.create!([{:cities => permissions.first.id, :categories => permissions.first.id, :cinemas => permissions.first.id, :cinema_films => permissions.first.id, :discount_sorts => permissions.first.id, :films => permissions.first.id,
                                ...
                                :ticket_sort_prices => permissions[3].id, :ticket_types => permissions[3].id, :users => permissions[3].id, :workers => permissions[3].id, :permissions => permissions[3].id, :receipts => permissions.first.id, :company_data => permissions[3].id, :selling_tickets => permissions.first.id},
                                ...
                                :ticket_sort_prices => permissions[0].id, :ticket_types => permissions[3].id, :users => permissions[3].id, :workers => permissions[1].id, :permissions => permissions[3].id, :receipts => permissions.first.id, :company_data => permissions[3].id, :selling_tickets => permissions.first.id}])
...
workers = Worker.create!([{:cinema_id => cinemas.first.id, :status_id => statusez.first.id, :login => 'gadmin', :hashed_password => 'wtw', :first_name => 'Jan', :last_name => 'Posypek', :phone => 123456789, :email => 'projekt_iz@o2.pl'},
                         {:cinema_id => cinemas.first.id, :status_id => statusez[2].id, :login => 'kasjer', :hashed_password => 'wtw', :first_name => 'Witek', :last_name => 'Kłos', :phone => 123456789, :email => 'projekt_iz@o2.pl'},
                         {:cinema_id => cinemas.first.id, :status_id => statusez[1].id, :login => 'kierownik', :hashed_password => 'wtw', :first_name => 'Ola', :last_name => 'Kłodzka', :phone => 123456789, :email => 'projekt_iz@o2.pl'}])
